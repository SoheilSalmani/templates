...
A dbt project on uv-managed Python 3.13 with dbt_utils, the house Agent Skills including the dbt ones, and an opinionated mise toolchain (Node 24, uv).
...
| --- | --- | --- | --- | --- |
| `project_name` | string | **yes** | — | Human-facing name; its snake_case slug names the dbt project and the Python package. (e.g. `Demo Service`) |
| `package_name` | string | no | example |  |
| `profile_name` | string | no | example | The profiles.yml entry dbt_project.yml points at; defaults to the project slug. (e.g. `demo_warehouse`) |
...
- **base**: Adds dbt_project.yml with dbt_utils in packages.yml, a uv-managed pyproject.toml pinned to Python 3.13, the empty models, macros and seeds directories, the dbt VS Code extension recommendation, and the stack .gitignore.
...
| --- | --- | --- | --- | --- | --- |
| pre | check | Verify uv is installed | `base` | `command -v uv` | — |
...
| post | setup | Lock and sync the Python environment | `base` | `uv lock && uv sync` | — |
| post | setup | Install dbt packages | `base` | `uv run dbt deps` | — |
