...
A FastAPI service on Python 3.13 managed by uv, with the house editor, git, mise, agent-instruction, skill and MCP configuration.
...
| --- | --- | --- | --- | --- |
| `project_name` | string | **yes** | — | Human-facing name; becomes the README title and, slugified, the package name in pyproject.toml. (e.g. `Demo Service`) |
...
- **base**: Adds a uv-managed FastAPI project on Python 3.13 with a main.py exposing three example routes, a README and the house .gitignore stanzas.
