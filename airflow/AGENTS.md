...
# airflow — template guide

An Apache Airflow project generated by the Astro CLI on Astro Runtime, with the house editor, git, mise, agent-instruction, skill and MCP configuration.
...
| --- | --- | --- | --- | --- |
| `project_name` | string | **yes** | — | Human-facing name; its slug names the Astro project in .astro/config.yaml. (e.g. `Demo Service`) |
...
- **base**: Adds the Astro CLI project layout: Dockerfile on Astro Runtime, the example DAG with its DAG-integrity tests, requirements.txt, packages.txt and the house .gitignore stanzas.
...
- **mise-astro**: Pins the Astro CLI 1.45 in mise.toml so mise install provides the astro binary that runs and tests the project locally.
...
| --- | --- | --- | --- | --- | --- |
| pre | check | Verify the Astro CLI is installed | `base` | `command -v astro` | — |
